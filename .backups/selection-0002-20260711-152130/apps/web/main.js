...
modules.register({
  manifest:{id:"region.box",version:"0.1.0",apiVersion:"region-v1",optional:false},
...
let renderer3d=null;
try{renderer3d=new ThreeRegionRenderer($("world"),command=>sandbox.dispatch(command));}
catch(error){showError(error);}
const outline=new OutlineRenderer($("outline-content"));

function refresh(state=sandbox.getState()){
  try{renderer3d?.update(state);}catch(error){showError(error);}
  try{outline.update(region,sandbox,modules.describe());}catch(error){showError(error);}
  $("undo").disabled=!sandbox.canUndo;
  $("redo").disabled=!sandbox.canRedo;
  $("review").disabled=!sandbox.dirty;
  $("status").textContent=`região ${region.version} · sandbox ${sandbox.dirty?"alterado":"limpo"}`;
}
sandbox.subscribe(state=>refresh(state));

document.querySelectorAll("[data-transform]").forEach(button=>{
  button.addEventListener("click",()=>renderer3d?.setTransformMode(button.dataset.transform));
});
$("space").addEventListener("click",event=>{
  const next=renderer3d?.toggleSpace();
  if(next)event.currentTarget.textContent=next==="world"?"Mundo":"Local";
});
$("add-box").addEventListener("click",()=>{
  const i=sandbox.getState().objects.length+1;
  sandbox.dispatch({type:"object.create",id:crypto.randomUUID(),name:`Caixa ${i}`,position:[0,1,-i],color:"#8a78d1"});
});
...
$("review").addEventListener("click",()=>{
  ...
  $("review-content").textContent=JSON.stringify(proposal,null,2);
...
$("confirm-proposal").addEventListener("click",()=>{
  ...
  if(result.accepted){
    $("status").textContent=`proposta aceita · região ${region.version}`;
  }
});
