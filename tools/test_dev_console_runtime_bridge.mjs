import {
  DevConsole
} from "../packages/devtools/src/DevConsole.js?build=20260806-0050a1";
...
if (entries.some(entry => !entry.ok)) {
  throw new Error(
    `Falha na execução: ${JSON.stringify(entries)}`
  );
}
...
if (
  ...
  throw new Error(
    `Consultas divergentes: ${JSON.stringify(queryCalls)}`
  );
}

const expectedCommands = [
  {
    id: "time.domain.pause",
    args: {
      id: "slow"
    }
  }
...
if (
  ...
  throw new Error(
    `Comandos divergentes: ${JSON.stringify(commandCalls)}`
  );
}

console.log(
  "DevConsole runtime bridge: 3/3 testes aprovados."
);
