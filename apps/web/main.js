...
const reducer =
  reducers.get("box-region");

if (!reducer) {
  throw new Error(
    "Reducer box-region unavailable"
  );
}
...
try {
  renderer3d =
    new ThreeRegionRenderer(
      $("world"),
      {
        dispatch: command =>
          sandbox.dispatch(command),
...
$("add-box").addEventListener(
  ...
  () => {
    const i =
      sandbox.getState().objects.length + 1;
...
$("confirm-proposal").addEventListener(
  "click",
  () => {
    const proposal =
      sandbox.createProposal();
    ...
    const result =
      region.acceptProposal(proposal);
