...
const APPLICATION_ROOT = new URL("./", self.registration.scope);
const REPOSITORY_ROOT = new URL("../../", APPLICATION_ROOT);
const CACHEABLE_ROOTS = [
  APPLICATION_ROOT,
  new URL("packages/", REPOSITORY_ROOT),
  new URL("vendor/", REPOSITORY_ROOT)
].map(url => url.href);
...
async function installApplication() {
  ...
  const urls = precache.files.map(resolveFromRepository);
...
async function networkFirst(request) {
  ...
  try {
    ...
    if (request.mode === "navigate") {
      const shell = await cache.match(resolveFromRepository(APP_SHELL));
...
async function fetchJson(path) {
  const response = await fetch(resolveFromRepository(path), { cache: "no-store" });
...
function isApplicationRequest(url) {
  return CACHEABLE_ROOTS.some(root => url.href.startsWith(root));
}
